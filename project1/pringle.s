...
pcmp3:
CBZ X8, pcontinuescan       // if we're out of arguments, we can't put anything here. return.
SUB X9, X9, 1               // i--; we are now overwriting the position of the %
SUB X8, X8, 1               // spec_count--
arrtoascii:
...
LDR X6, [SP, 56]
LDR X7, [SP, 64]
LDR X8, [SP, 72]            // X8 = remaining specs
LDR X10, [SP, 88]
LDR X11, [SP, 96]
LDR X12, [SP, 104]
...
STR X0, [SP, 8]             // protect X0-X6. we're not using X7-X18 yet.
STR X1, [SP, 16]
STR X2, [SP, 24]
STR X3, [SP, 32]
STR X4, [SP, 40]
STR X5, [SP, 48]
STR X6, [SP, 56]
BL count_specs              // count amount of specifiers
MOV X8, X0                  // X8 = remaining specifiers
LDR X0, [SP, 8]             // load X0-X6. we're not using X7-X18 yet.
LDR X1, [SP, 16]
LDR X2, [SP, 24]
LDR X3, [SP, 32]
LDR X4, [SP, 40]
...
MOV X9, 0                   // X9 = iterator for the string
MOV X12, 0                  // X12 = length of output string
ADR X13, pstr               // X13 = &pstr
pscanloop:
  LDRB W10, [X0, X9]        // X10 = str[i]
  STRB W10, [X13, X9]       // pstr[i] = str[i]
  ADD X9, X9, 1             // i++
  LDRB W11, [X0, X9]        // X11 = str[i]
